...
# Otros
COS=$(echo "displayName zimbraAccountStatus zimbraCreateTimestamp")
...
echo "email;displayName;Status;Created;Description;COSid;LastLogon;Size;Alias" >> $FOLDER/$LISTADO
...
for EMAIL in ${CUENTAS}; do
  ...
  then
    MEMALIAS=`zmprov -l ga $EMAIL zimbraMailAlias | grep -v "#" | awk -F "zimbraMailAlias: " '{print $2}' | grep . | awk 'ORS=";"' | head -c-1`
        echo "$EMAIL;$RESULTADO;$DESCRIPTION;$COSID;$LASTLOGON;$MB_SIZE;$MEMALIAS" >> $FOLDER/$LISTADO
  else
    echo "$EMAIL;$RESULTADO;$DESCRIPTION;$COSID;$LASTLOGON;$MB_SIZE" >> $FOLDER/$LISTADO
  fi
done
